...
- **TC-13:** Email case-insensitive uniqueness — Given a Microsoft Entra account whose `email` claim is sent as `Mike@Nortal.com` on the very first sign-in (stored as `mike@nortal.com` — see implementation note below), when the same `oid` later signs in with email claim `MIKE@NORTAL.COM`, then the existing participant row is matched (lookup by `oid`), no duplicate row is created, and the stored email remains `mike@nortal.com`. {Source: AI/Clarify}
  - **Implementation note**: lowercase storage is delivered by Supabase Auth normalising `auth.users.email` at user-creation time, so the JWT's `email` claim always arrives lowercase by the time the provisioning RPC runs. The `participants.email` column is `citext` (case-insensitive comparisons) and a BEFORE INSERT/UPDATE trigger trims surrounding whitespace; together they provide defence in depth, but the observable lowercase result comes from the Auth-layer normalisation. A whitespace-padded `email` variant (`'  mike@nortal.com  '`) is unreachable through the OAuth flow because Supabase Auth's email-format validator rejects whitespace at `auth.admin.createUser` time; the trigger's whitespace branch is exercised by direct INSERTs in pgTAP `004_email_normalization.sql`.
