...
**RankWidget multi-mount channel collision.** `RankWidget.tsx` was opening a Supabase Realtime channel under the fixed topic `leaderboard-refresh`. DashboardPage renders the Today widget tree twice (mobile panel + desktop grid), so two RankWidget instances mount and `supabase.channel()` coalesces by topic — the second mount's `.on()` lands on an already-SUBSCRIBED channel and throws `cannot add postgres_changes callbacks for realtime:leaderboard-refresh after subscribe()`. The pre-tournament path early-returned before subscribe, which is why feature 004's tests never tripped this. The fix attaches a per-mount unique suffix via `useId()`. Committed in `7a6363c`.

**SnapshotWidget PGRST200 last-finished embed.** `SnapshotWidget.tsx`'s last-finished query started `FROM predictions` and tried to embed `score_events`. PostgREST cannot resolve a relationship between `predictions` and `score_events` (no FK between them; they share only `matches.id` / `participants.id`), so the embed failed with PGRST200 (`Could not find a relationship between 'predictions' and 'score_events' in the schema cache`) and `lastRes.data` always returned `null`. The widget rendered the empty-state copy on every dashboard load even when the participant had predictions on finished matches. The fix inverts the query — it now starts `FROM matches` and reverse-embeds both `predictions!inner` (filtered to the caller's participant_id) and `score_events` (filtered to the caller's participant_id). Both embeds resolve cleanly because each table has a direct FK to `matches.id`. Identified during the manual verification pass and fixed in this commit.
...
| FR-D09 | Snapshot widget: last finished + next upcoming side-by-side / stacked | `components/dashboard/SnapshotWidget.tsx` (last-finished query starts FROM `matches` with reverse-embedded `predictions!inner` + `score_events`; PGRST200 join bug fixed in this commit — see Pre-existing bugs fixed) | covered by `dashboard-inline-edit.spec.ts` interactive path; widget renders inline within Today widgets; manual reload verifies zero console errors |
