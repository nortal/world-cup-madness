    ...
    try {
      ...
      if (error !== null) {
        ...
        if (code === '23514' && message.includes('PREDICTION_LOCKED')) {
          setErrorCode('errorLocked');
        } else if (code === '23514') {
          setErrorCode('errorOutOfRange');
        } else if (code === 'P0002' && message.includes('MATCH_NOT_FOUND')) {
          setErrorCode('errorMatchNotFound');
        } else if (code === 'P0002') {
          setErrorCode('errorParticipantNotFound');
        } else {
  ...
  return (
    <section aria-labelledby="prediction-form-heading" className="mt-6 rounded-md border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <form onSubmit={onSubmit} className="mt-4 space-y-4" noValidate>
        <div className="flex items-end gap-4">
          <label className="flex flex-col">
            ...
            <input
              ...
              aria-describedby={errorCode !== null ? 'prediction-form-error' : undefined}
        ...
        <PredictionFormError errorCode={errorCode} id="prediction-form-error" />
      </form>
    </section>
  );
}
